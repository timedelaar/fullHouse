...
public class SpelerView extends javax.swing.JPanel {
    ...
    public SpelerView(MainWindow window, Connection conn) {
        initComponents();
        this.window = window;
        this.conn = conn;
        getSpelers();
    ...
    private void editSpelerBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_editSpelerBtnMouseClicked
        ...
        if (row.length > 1) {
            ...
            Speler speler = getSpelerFromTable(row[0]);
            EditSpeler editSpeler = new EditSpeler(this, speler);
            editSpeler.setLocation(200, 150);
    ...
    private void deleteSpelerBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_deleteSpelerBtnMouseClicked
        int confirm = JOptionPane.showConfirmDialog(null, "Weet U zeker dat U deze speler wilt verwijderen?", "Verwijder speler?", JOptionPane.YES_NO_OPTION);
    ...
    private void addInschrijvingBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_addInschrijvingBtnMouseClicked
        ...
        if (row.length > 1) {
            ...
            Speler speler = getSpelerFromTable(row[0]);
            AddInschrijving inschrijving = new AddInschrijving(speler, conn);
            inschrijving.setLocation(200,100);
    ...
    public void addSpeler (Speler speler) {
        String query = "INSERT INTO Speler(naam, voorletters, straatnaam, huisNr, woonplaats, postcode, telefoonNr, email, rating, winsten, docent)"
                + "VALUES(?,?,?,?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement stat = conn.prepareStatement(query);
            stat.setString(1, speler.naam);
            stat.setString(2, speler.voorletters);
            stat.setString(3, speler.straatnaam);
            stat.setString(4, speler.huisNr);
            stat.setString(5, speler.woonplaats);
            stat.setString(6, speler.postcode);
            stat.setString(7, speler.telefoonNr);
            stat.setString(8, speler.email);
            stat.setInt(9, speler.rating);
            stat.setInt(10, speler.winsten);
            stat.setBoolean(11, speler.docent);
    ...
    public void fillTable(ResultSet result) throws SQLException {
        ...
        while (result.next()) {
            ...
            String winsten = result.getString("winsten");
            Boolean docent = result.getBoolean("docent");
    ...
    public void editSpeler (Speler speler) {
        String query = "UPDATE Speler SET naam=?, voorletters=?, straatnaam=?, huisNr=?, woonplaats=?, postcode=?,"
                + "telefoonNr=?, email=?, rating=?, winsten=?, docent=? WHERE spelerID=?;";
        try {
            PreparedStatement stat = conn.prepareStatement(query);
            stat.setString(1, speler.naam);
            stat.setString(2, speler.voorletters);
    ...
    private Speler getSpelerFromTable (int row) {
        ...
        int winsten = Integer.parseInt(spelerTable.getValueAt(row, 10).toString());
        boolean docent = Boolean.parseBoolean(spelerTable.getValueAt(row, 11).toString());
        Speler speler = new Speler(id, naam, voorletters, straatnaam, huisNr, woonplaats, postcode, telefoonNr, email, rating, winsten, docent);
        return speler;
    }
